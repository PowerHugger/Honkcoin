...
#if defined(NDEBUG)
# error "Honkcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("7c6912d0652d4b407eb1db706a78f2e8601a4010a40c80e2a1f1aff17fbb4394");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Honkcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Honkcoin Signed Message:\n";
...
{
    // Honkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Honkcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Honkcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Honkcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Honkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Honkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("c8ecb8674ef23714fdc82332e96f0436a714db94c5556a6a0526abf26ee958e0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "I honked today";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0470a69b59dd5271ae2aa4cb311d45a9e31f2a91b3fb07d3a1f583bb491cb50f1d5ef6f080fbefbed9fce864b5ee0a20681340baea12f83d80763bf46e29bcf364") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519115119;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086173237;

        if (fTestNet)
        {
            block.nTime    = 1519115095;
            block.nNonce   = 386223605;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("92d48e72bee944e9dce79f656fb07c3cf3e1fc3460c7da942b2ab612946471c9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xca, 0xb4, 0xd2 }; // Honkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HonkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HonkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HonkcoinMiner(CWallet *pwallet)
{
    printf("HonkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("honkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HonkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HonkcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HonkcoinMiner, pwallet));
}
